...
- Added an actionable first-run panel with direct Open Image and New Canvas controls.
- Replaced letter fallback tools with a consistent icon family and exposed readable control names to assistive technology.
- Made tool options contextual, with brush dynamics collapsed until they are needed.
- Refined the dark workspace, rulers, selection outline, dock headings, and status bar for clearer visual hierarchy.
- Rebuilt the GitHub landing page around four private-desktop captures and new product artwork.
- Added a repeatable screenshot workflow plus a one-file Windows build with SHA-256 checksums.
- Changed the project license to GPL-3.0 so packaged releases follow PyQt5's redistribution terms.
